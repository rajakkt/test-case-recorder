...
function Build-TestStepsItems {
  ...
  foreach ($step in $SourcePayload.testScript.steps) {
    $stepIndex = 1
    ...
    $inlineStep = [ordered]@{
      ...
      expectedResult = To-SafePlainText -Text ([string]$step.expectedResult) -MaxLength 300 -Fallback "Step completes successfully"
    }
...
$hardcodedEnvPath = Join-Path (Split-Path -Parent $scriptRoot) ".env"
...
function Build-PayloadForUpload {
  ...
  $stepItems = Build-TestStepsItems -SourcePayload $payload
